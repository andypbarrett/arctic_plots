...
def get_weight(region='Nof80'):
    ...
    lwgt = np.cos(mask['lat'] * np.pi/180.)
    # deal with floating point error for cos(90.)
    # N.B. this means no gridcells as 90N or 90S are included in weighted averages
    latwgt = xr.DataArray( np.where( np.isclose(0., lwgt), 0., lwgt ), [mask.coords['lat']] )
    print (mask['lat'][0], lwgt[0])
...
def main():
    ...
    def process_one_path(path):
        with xr.open_dataset(path) as ds:
            ds = (ds['T'][4,:,:] * weight).sum(dim=['lat_0','lon_0'])
            ds.load()
        return ds

    datasets = [process_one_path(p) for p in fileList]
    tavg = xr.concat(datasets, 'time')

    tavg['time']=[dt.datetime.strptime(f.split('.')[4],'%Y%m%d') for f in fileList]
    tavg.time.encoding['units'] = 'days since 1900-01-01 00:00:00'
    tavg.name = 'T925'

    filo = reanalysis+'.T925.arctic.avg.'+repr(year)+'.'+region+'.test.nc'
